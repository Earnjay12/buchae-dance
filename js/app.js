(function () {
  const D = window.CHOREO, plan = window.PLAN.build(D, window.PLAN_ROUTES);
  const FPS = D.video.fps, DUR = D.video.duration, BEAT = plan.BEAT;
  const $ = s => document.querySelector(s), $$ = s => [...document.querySelectorAll(s)];
  const store = { get(k, d) { try { const v = localStorage.getItem("buchae19." + k); return v === null ? d : JSON.parse(v); } catch (e) { return d; } }, set(k, v) { try { localStorage.setItem("buchae19." + k, JSON.stringify(v)); } catch (e) { } } };

  const S = {
    t: 5, playing: false, speed: 1, src: "none", hi: store.get("hi", null), filter: "all", mode: store.get("mode", "top"),
    set: Object.assign({ W: 10, Dp: 6, step: 0.5, view: "student" }, store.get("set", {})), beatIdx: -1, loop: false
  };
  ...
  const sx = () => S.set.W / 10, sy = () => S.set.Dp / 6;
  const badge = id => `<span class="badge ${id > 10 ? "b" : ""}" title="${id <= 10 ? "여" : "남"}">${id}</span>`;
  function lr(x) { const left = x < 3.4, right = x > 6.6; if (!left && !right) return "가운데"; return (left ^ !aud()) ? "왼쪽" : "오른쪽"; }
  function zone(x, y) {
    if (x < -0.2) return aud() ? "무대 밖 (왼쪽 옆)" : "무대 밖 (오른쪽 옆)";
    if (x > 10.2) return aud() ? "무대 밖 (오른쪽 옆)" : "무대 밖 (왼쪽 옆)";
    const fb = y > 4.3 ? "앞" : y > 2.6 ? "가운데" : "뒤", side = lr(x);
    if (fb === "가운데" && side === "가운데") return "무대 한가운데";
    if (side === "가운데") return `${fb}쪽 가운데`;
    if (fb === "가운데") return `가운데 줄 ${side}`;
    return `${fb}쪽 ${side}`;
  }
  const coord = (x, y) => `가로 ${(Math.max(0, Math.min(10, x)) * sx()).toFixed(1)}m · 앞에서 ${((6 - y) * sy()).toFixed(1)}m`;
  ...
  function moveDesc(id, beat) {
    ...
    const db = Math.round(m.delay / BEAT);
    ...
    return {
      ...
      html: `<span class="mv">${parts.join(" → ")}</span><span class="sm">→ ${zone(dest[0], dest[1])} (${coord(dest[0], dest[1])})</span>` +
        `<span class="pass">${fmtS(tr.t0 + m.delay)} ${start} · ${fmtS(tr.t1)}까지 도착${near.length ? " · " + near.join(", ") : ""}${tight ? ` · <span class="warn">좁은 곳: 부채를 가슴에 붙이기</span>` : ""}</span>`,
  ...
  // 설정
  $("#stW").value = S.set.W; $("#stD").value = S.set.Dp; $("#stStep").value = S.set.step;
  $$('input[name=view]').forEach(r => r.checked = r.value === S.set.view);
  const saveSet = () => { S.set = { W: +$("#stW").value || 10, Dp: +$("#stD").value || 6, step: +$("#stStep").value || 0.5, view: ($('input[name=view]:checked') || {}).value || "student" }; store.set("set", S.set); render("beat"); render(true); renderStudent(); renderSafety(); };
  ["#stW", "#stD", "#stStep"].forEach(s => $(s).addEventListener("change", saveSet));
  ...
  function renderSafety() {
    ...
    $("#p-safety").innerHTML = `<p class="safe-sum">${hard === 0 ? "✅ 모든 전환에서 몸 중심 사이 50cm 이상 떨어져요 (부딪힘 0건)" : `⚠ 부딪힐 수 있는 곳 ${hard}건`}</p>
      <p class="note">4학년이 서로 부딪히지 않도록 이동 경로를 계산했어요. 먼저 출발할 사람, 기다렸다 출발할 사람, 뒤로 돌아가는 사람을 정리했어요. 점선 화살표가 이동 경로예요.</p><div class="safe">` +
      plan.trans.map(tr => {
        const A = plan.forms[tr.from], B = plan.forms[tr.to];
        const special = Object.values(tr.movers).filter(m => m.moving && (m.route !== "s" || m.delay > 0));
        ...
        return `<div class="sec"><h3><span class="t">${fmtS(tr.t0)}~${fmtS(tr.t1)}</span>${A.name} → ${B.name}</h3>
          <ul>${special.length ? special.map(m => `<li>${badge(m.id)} ${m.delay ? `<b>${Math.round(m.delay / BEAT)}박 기다렸다</b> 출발${m.waitFor ? ` (${m.waitFor}번 먼저)` : ""}` : "바로 출발"}${m.route !== "s" ? " · " + (m.route === "bk" ? "뒤쪽 길로 돌아가기" : m.route === "fr" ? "앞쪽 길로 돌아가기" : m.route === "hv" || m.route === "vh" ? "ㄱ자로 꺾어 가기" : "둥글게 돌아가기") : ""}</li>`).join("") : "<li>모두 곧게 이동</li>"}
  ...
  // 연습 순서
  $("#p-practice").innerHTML = `<div class="practice card"><h3>연습 순서 제안 (4학년 19명)</h3>
    <ol>
      <li><b>기본 부채 동작 6가지</b>를 자리에서 먼저: 펴기·접기 → 가슴 수평 물결 → 머리 위 45°(V자) → 머리 위 지붕 → 얼굴 앞 동그라미(꽃송이) → 옆 날개. '부채 동작 사전' 탭의 그림을 보며 해요.</li>
      <li><b>바닥 표시</b>: 무대에 가로 1m마다 번호 테이프(0~10), 앞에서 1m·2m·3m 줄을 붙이면 '가로 4.5m · 앞에서 1.2m'를 바로 찾을 수 있어요.</li>
      ...
      <li><b>특별 역할</b>: 8·9번 회전, 4번 층층 부채, 4·10번 듀엣, 19번 가운데 기둥은 따로 연습한 뒤 합쳐요.</li>
  ...
  window.BUCHAE = { seek, plan, state: S };
})();
